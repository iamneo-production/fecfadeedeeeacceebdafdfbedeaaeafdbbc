...
const Card=({question ,correctAnswerMarkUpdate ,attempt , options, answer, setQsns,qsn,setCard,qsnAttempt})=>{
    const optionArray =[options.option1,options.option2,options.option3,options.option4];
    ...
    return(
        <div>
            ...
            <div className="flex space-x-2">
                ...
                <Button key={index} marked={marked} onclick={()=> click(option)} style={!marked ? norm:disabled}>{option}</Button>)}
            </div>
        </div>
    )
}
